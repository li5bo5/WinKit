...
* **10 秒临时唤出**：支持全局快捷键临时唤出，无操作 10 秒后自动平滑收起。
* **极速编辑与退出**：内联输入框支持 `Win + S` / `Ctrl + S` 快捷保存退出，输入框外部双击快速退出编辑。
* **轻量本地持久化**：待办条目采用标准 Markdown 格式实时同步至本地 `%AppData%\WinKit\todos.md`。

### 2. 📋 Clipboard (剪贴板历史)
* **无焦点弹窗 (`Win + V`)**：弹出无焦点悬浮面板，**在资源管理器重命名文件时按 Win+V 绝不上夺焦点，重命名编辑框不中断**。
* **固定模式 (Pin 侧边栏)**：支持将剪贴板固定在最上层，**双击/单击复制粘贴后窗口不消失**，支持侧边栏连续批量回填粘贴。
* **双击自动粘贴 (回填)**：精准恢复前台工作窗口焦点，模拟 `Ctrl + V` 物理击键回填至原输入框。
* **失焦智能隐藏**：未固定模式下，点击外部任意程序区域 100% 自动隐藏。
* **滚动位置记忆**：托盘提供“记忆滚动”配置开关。默认未开启时，每次打开自动复位显示顶部第一条；开启后记忆上次滚动位置。

### 3. ⌨️ 全局自定义快捷键系统
* **托盘快捷键面板**：支持物理按键捕获与热重载，可自由配置“临时显示 TodoList”、“唤出/隐藏剪贴板”、“截图 OCR 映射”、“系统截图工具”、“保存退出编辑”等热键。

### 4. 🎨 现代自研 Fluent 托盘右键菜单
* **WPF 上下文菜单接管**：采用 WPF 自研设计系统，带 8px 圆角、微透明磨砂白底色 (#F2F5F5F5)、高档投影与 hover 动态高亮。
* **层级结构**：TodoList 菜单、Clipboard 菜单、快捷键设置、不透明度调节、开机自启与关于。
...
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained false -o bin\Publish\FrameworkDependent_V2.1

# 2. 编译并打包：独立免装版 (Self-Contained)
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained true -o bin\Publish\SelfContained_V2.1
```
