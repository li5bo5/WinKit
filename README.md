...
`WinKit` 是一款专为 Windows 设计的、极致轻量且美观的效率工具箱。它将 **TodoList (待办工具)** 与 **Clipboard (高级剪切板)** 深度整合，采用高档磨砂半透明（毛玻璃）视觉体系，并自研了 100% 贴合 Windows 11 Fluent 风格的系统托盘右键菜单。
...
### 1. 🎨 智能主题色彩与自适应视觉
* **任务栏色彩自动跟随**：窗口背景色智能跟随 Windows 任务栏颜色与深浅模式，自动融合系统强调色（Accent Color），系统主题变更时 0 延迟实时同步。
...
* **键盘防误触机制**：禁用辅助按钮的键盘聚焦，杜绝空格键误触关闭窗口，原生保障回车确认与 Esc 取消。
...
### 4. ⚙️ 统一偏好设置中心
* **全功能配置面板**：360×560 卡片式磨砂面板，集中整合“待办清单”、“高级剪贴板”、“常规与外观（主题颜色、不透明度与开机启动）”全部配置项，支持快捷键物理捕获、即时保存与一键恢复默认。
...
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained false -o bin\Publish\FrameworkDependent_V2.4

# 2. 编译并打包：独立免装版 (Self-Contained)
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained true -o bin\Publish\SelfContained_V2.4
```
