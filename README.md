...
WinKit 遵循 Windows 官方标准规范，所有配置与数据均存储于用户的 `AppData\Roaming` 目录中。

* **数据根目录**：`%APPDATA%\WinKit`（即 `C:\Users\<您的用户名>\AppData\Roaming\WinKit`）
* **快速直达**：按下键盘快捷键 <kbd>Win</kbd> + <kbd>R</kbd>，在运行窗口输入 `%APPDATA%\WinKit` 并回车即可直接打开该目录。

### 文件结构清单

| 文件名 | 存储格式 | 用途说明 | 特性与编辑建议 |
| :--- | :---: | :--- | :--- |
| **`settings.json`** | JSON | **全软件统一配置文件** | 存储快捷键、窗口不透明度、双击托盘联动开关、保留天数/条数等偏好设置。 |
...
> 💡 **备份提示**：如需备份数据或迁移至新电脑，只需复制整个 `%APPDATA%\WinKit` 文件夹即可完成 100% 完整迁移。每个数据文件均附带 `.bak` 损坏自愈备份机制。
