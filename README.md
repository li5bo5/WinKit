...
`WinKit` 是一款专为 Windows 设计的、极致轻量且美观的效率工具箱。它将 **TodoList (待办工具)**、**Clipboard (高级剪贴板)** 、 **Quick Phrases (常用短语)** 与**Quick Launch (快捷启动)** 深度整合，采用高档磨砂半透明（毛玻璃）视觉体系，并自研了 100% 贴合 Windows 11 Fluent 风格的系统托盘右键菜单。
...
### 5. 🚀 QuickLaunch (快捷启动)
* **绿色软件便携启动**：告别杂乱的桌面快捷方式，通过托盘二级菜单即可秒级直达常用绿色软件、脚本与常用文件。
* **轻量 JSONL 配置**：数据存放在本地 `%AppData%\WinKit\quick-launch.jsonl`，一行一条记录，便于手动编辑与脚本批量维护。
* **无缝快捷触发**：鼠标悬停至托盘“快捷启动”二级菜单自动展开列表，点击应用名称即可自动调用系统进程启动。

### 6. ⚙️ 统一偏好设置 (Preferences)
...
### ⚙️ 快捷启动配置文件说明 (`quick-launch.jsonl`)

配置文件路径：`%AppData%\WinKit\quick-launch.jsonl`
