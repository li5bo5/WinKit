...
* **双击自动粘贴**：精准恢复前台工作窗口焦点，模拟 `Ctrl + V` 物理击键回填至原输入框。
* **失焦智能隐藏**：未固定模式下，点击外部任意程序区域自动隐藏。
* **滚动位置记忆**：支持“记忆滚动”配置，默认未开启时，每次打开自动复位显示顶部第一条；开启后记忆上次滚动位置。

### 3. ⌨️ 全局自定义快捷键系统
* **托盘快捷键面板**：300×300 精巧正方形面板，支持物理按键捕获与热重载，可自由配置“置顶显示 (`Ctrl + D`)”、“唤出剪贴板 (`Win + V`)”、“保存退出编辑 (`Ctrl + S`)”等热键。

### 4. 🎨 现代自研 Fluent 托盘右键菜单
* **托盘菜单**：采用 WPF 、微透明磨砂白底色、动态高亮。
...
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained false -o bin\Publish\FrameworkDependent_V2.2

# 2. 编译并打包：独立免装版 (Self-Contained)
dotnet publish WinKit.csproj -c Release -r win-x64 --self-contained true -o bin\Publish\SelfContained_V2.2
```
