...
* **全局快捷键呼出**：默认按 `Ctrl + ~` 即可随时显示/隐藏待办窗口；快捷键可在软件目录下的 `config.json` 中自定义（见下文「快捷键配置」）。
...
TodoList 待办窗口支持全局快捷键一键显示/隐藏。快捷键配置文件位于软件目录（与 `WinKit.exe` 同级）下的 `config.json`，首次启动时会自动生成：
...
* 格式为 `修饰键 + 主键`，修饰键支持 `Ctrl`、`Shift`、`Alt`、`Win` 的任意组合（至少需要一个修饰键）。
* 主键支持字母 `A-Z`、数字 `0-9`、功能键 `F1-F24`，以及 `` ` ``/`~`、`Tab`、`Space`、`Enter`、`Esc`、方向键等常用按键。
* 修改后重启软件生效，例如改为 `"Alt + T"`、`"Ctrl + Shift + T"`、`"Win + ~"` 均可。
